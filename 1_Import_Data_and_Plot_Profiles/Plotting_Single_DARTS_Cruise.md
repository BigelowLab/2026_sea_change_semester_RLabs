...
David Fields has sent you an email, where he has attached an [excel
file](https://docs.google.com/spreadsheets/d/1iZ4HbfRHGD-cPz6SCrBwKZYPxMUwdORl/edit?usp=sharing&ouid=100534300918978737444&rtpof=true&sd=true)
from your most recent DaRTS cruise. His email is a little cryptic but
says:

    Using R...
       1) plot TEMP, SALINITY, DENSITY, CHL, O2(%SATURATION) AND PH  in R with the Depth (m) on the vertical axis in reverse order.

Being a resourceful student, you open up the excel file to look at the
data yourself, and find that it’s not exactly ready to load into R. *Why
is this?*

Some things you notice:

- **Embedded graphs**: It’s nice that David added these
  pre-visualizations, but we won’t be able to load these into R.

- **Multiple tabs**: Each station is represented by it’s own tab in the
  spreadsheet. This will be a problem when we want to compare stations.
  It would be better to have all stations in the same table, with a
  column designating the station.

- **Column headers**: The column headers have too many extra spaces and
  characters, which could cause problems when referencing them with R.

- **Dates**: There’s no indication of when these data were collected
  outside of the name of the initial file. What if you wanted to merge
  data from multiple cruises? A date column is important!

#### Cleaning Up the Data

Let’s create a spreadsheet we will actually be able to load into R. We
will start by creating a new tab in the Excel Spreadsheet. We will call
this tab “combined”. Then, we will add headers to the first row of this
new table. The first two headers will be **date** and **station**, and
then we will add headers for the measurement columns on the station
tabs, but we will rename them slightly to be more coding/R friendly.
...
Next, we will copy and paste the data from each spreadsheet into this
new table.

Be sure to skip the header row when you copy and paste, and make sure
that the first column you paste is below the “depth_m” column.

With each tab, we will also need to make sure that the station number is
transferred to the new spreadsheet in the “station” column, so when you
copy and paste all the data from the “S1” tab, population the
corresponding rows in the “station” column with “S1”.

The date will be 240905 throughout this whole spreadsheet, as all this
data comes from the cruise that set out on that particular day.

Once you have all the data organized into one spreadsheet, you can
export it as a csv to a known location on your computer. A good place
might be in a “darts_data” folder on your desktop. Let’s name the file
“240905_darts_clean.csv”

## Loading your data into R

Now that we have a clean spreadsheet, we can load it into R. Before that
we’ll want to load the R libraries we’ll be using today. We’re going to
load a large package that has a lot of other packages within it, called
the “tidyverse”:
...
Let’s try to read in the file we created. The function to do this is
`read.csv`, and we need to provide the name and location of our data
file, including the path to the file if it is in a different location
than this markdown file.
...
In the below `"~/Desktop"` indicates that you’re starting in your home
directory and then moving to your Desktop.
...
```
    ...
    ##  $ date             : int  20240905 20240905 20240905 20240905 20240905 20240905 20240905 20240905 20240905 20240905 ...
    ##  $ station          : chr  "S1" "S1" "S1" "S1" ...
    ...
    ##  $ density_kg_per_m3: num  22.7 22.7 22.7 22.7 22.7 ...
    ##  $ par              : num  1400 1700 1670 1790 1640 1660 1490 1560 1430 1550 ...
    ##  $ fluor_mg_per_m3  : num  0.978 0.984 1.007 1.011 1.052 ...
    ##  $ turbidity_ntu    : num  1.16 1.15 1.15 1.15 1.15 ...
    ##  $ O2_umol_per_kg   : num  200 201 201 201 202 ...
    ##  $ O2_pct_sat       : num  81.7 81.8 81.8 81.8 82.3 ...
...
```
    ...
    ##       date station depth_m  temp_c sal_psu density_kg_per_m3  par
    ## 1 20240905      S1   0.780 16.8495 31.3065           22.7164 1400
    ## 2 20240905      S1   0.787 16.8444 31.3016           22.7138 1700
    ## 3 20240905      S1   0.789 16.8368 31.3022           22.7160 1670
    ## 4 20240905      S1   0.790 16.8294 31.3028           22.7182 1790
    ## 5 20240905      S1   0.792 16.8240 31.3023           22.7190 1640
    ## 6 20240905      S1   0.791 16.8298 31.2936           22.7110 1660
    ##   fluor_mg_per_m3 turbidity_ntu O2_umol_per_kg O2_pct_sat    ph
...
![](Plotting_Single_DARTS_Cruise_files/figure-gfm/unnamed-chunk-6-1.png)<!-- -->
...
![](Plotting_Single_DARTS_Cruise_files/figure-gfm/unnamed-chunk-7-1.png)<!-- -->
...
![](Plotting_Single_DARTS_Cruise_files/figure-gfm/unnamed-chunk-8-1.png)<!-- -->
...
![](Plotting_Single_DARTS_Cruise_files/figure-gfm/unnamed-chunk-10-1.png)<!-- -->
...
![](Plotting_Single_DARTS_Cruise_files/figure-gfm/unnamed-chunk-11-1.png)<!-- -->

### Single plot for all stations

Here, we add an additional argument to the call to `aes` to plot each
station’s data a different color:
...
![](Plotting_Single_DARTS_Cruise_files/figure-gfm/unnamed-chunk-12-1.png)<!-- -->
...
![](Plotting_Single_DARTS_Cruise_files/figure-gfm/unnamed-chunk-13-1.png)<!-- -->

# Practice Questions

1.  Plot salinity, density and O2 profiles for each station using
    faceting
