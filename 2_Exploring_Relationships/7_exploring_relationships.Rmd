---
title: 'Investigating Relationships'
output:
...
3. Save the new script e.g. `7_exploring_relationships.rmd`
...
More quick and dirty visualization tricks... filtering via pipe and then plotting:

```{r}
data %>% filter(Temperature_C < 20) %>%
  ggplot() + 
  geom_point(mapping = aes(x = Temperature_C, y = Fluorescence)) +
  xlab('Temperature (deg C)') +
  ylab('Fluorescence (mg/m^3)') +
...
```{r}
data %>% filter(Temperature_C > 20) %>% group_by(Date) %>% summarise(count = n(), mean = mean(Depth))
```


For the purposes of this tutorial, let's ignore the outlier temperatures for now:

```{r}
data <- data %>% filter(Temperature_C < 20)
```
...
corValue
```
The correlation coefficient is the number right at the bottom of the printout, below the word `cor`. In this case, we have a correlation coefficient of 0.356503.
...
Here, the low pvalue suggests that we can reject the null hypothesis that there is no correlation between the two variables. The relatively low but positive correlation coefficient indicates that there is a weak but significant relationship between fluorescence and temperature.
...
ggplot(data = data) + 
  ...
  ylab('Fluorescence (mg/m^3)') +
  scale_y_log10() +
  annotate("text", x = 12, y = 15, label = paste("Temp = ", format(slopeLog, digits=3), " log10(Fluor) + (", format(interceptLog,digits=3), ")", sep="")) # let's add the equation to the figure
```
